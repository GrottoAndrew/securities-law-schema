...
const EVIDENCE_TEMPLATES = {
  'accredited-investor': [
    {
      title: 'Accredited Investor Questionnaire - {client}',
      fileType: 'pdf',
    ...
    {
      title: 'CPA Certification of Accredited Status - {client}',
      fileType: 'pdf',
      sizeRange: [40000, 150000],
    },
    { title: 'Bank Statement Summary - {client}', fileType: 'pdf', sizeRange: [100000, 500000] },
    {
      title: 'Brokerage Account Statement - {client}',
      fileType: 'pdf',
      sizeRange: [80000, 400000],
    },
  ...
  disclosure: [
    {
      title: 'Private Placement Memorandum v{version}',
      fileType: 'pdf',
      sizeRange: [500000, 2000000],
    },
  ...
  communication: [
    {
      title: 'Client Communication Log - {month} {year}',
      fileType: 'xlsx',
      sizeRange: [50000, 300000],
    },
    {
      title: 'Email Archive - {client} - {quarter}',
      fileType: 'zip',
      sizeRange: [1000000, 10000000],
    },
    { title: 'Meeting Notes - {client} - {date}', fileType: 'docx', sizeRange: [15000, 60000] },
    {
      title: 'Phone Call Recording Transcript - {client}',
      fileType: 'txt',
      sizeRange: [5000, 30000],
    },
    { title: 'Marketing Material - {campaign}', fileType: 'pdf', sizeRange: [200000, 1000000] },
    {
      title: 'Investor Update Letter - {quarter} {year}',
      fileType: 'pdf',
      sizeRange: [100000, 500000],
    },
    {
      title: 'Webinar Recording Transcript - {topic}',
      fileType: 'txt',
      sizeRange: [20000, 100000],
    },
  ],
  'form-d': [
  ...
  integration: [
    ...
    {
      title: 'SEC Disciplinary History Search - {date}',
      fileType: 'pdf',
      sizeRange: [20000, 80000],
    },
  ...
  solicitation: [
    ...
    {
      title: 'Pre-existing Relationship Documentation - {client}',
      fileType: 'docx',
      sizeRange: [20000, 80000],
    },
    { title: 'General Solicitation Compliance Memo', fileType: 'pdf', sizeRange: [40000, 150000] },
    {
      title: 'Advertising Review Log - {month} {year}',
      fileType: 'xlsx',
      sizeRange: [30000, 120000],
    },
    {
      title: 'Social Media Compliance Audit - {quarter}',
      fileType: 'pdf',
      sizeRange: [100000, 400000],
    },
  ],
  'record-retention': [
    ...
    {
      title: 'Archive Inventory Report - {quarter} {year}',
      fileType: 'csv',
      sizeRange: [50000, 300000],
    },
  ...
  audit: [
    {
      title: 'Internal Audit Report - {quarter} {year}',
      fileType: 'pdf',
      sizeRange: [200000, 800000],
    },
    ...
    {
      title: 'User Activity Report - {month} {year}',
      fileType: 'xlsx',
      sizeRange: [80000, 400000],
    },
  ],
  training: [
    {
      title: 'Employee Training Completion Report - {year}',
      fileType: 'xlsx',
      sizeRange: [30000, 150000],
    },
    {
      title: 'Compliance Training Materials - {topic}',
      fileType: 'pdf',
      sizeRange: [500000, 2000000],
    },
    { title: 'Training Acknowledgment - {employee}', fileType: 'pdf', sizeRange: [10000, 40000] },
    {
      title: 'Annual Certification - {employee} - {year}',
      fileType: 'pdf',
      sizeRange: [15000, 50000],
    },
  ...
  'aml-kyc': [
    ...
    {
      title: 'Enhanced Due Diligence Report - {client}',
      fileType: 'pdf',
      sizeRange: [100000, 500000],
    },
  ...
  cybersecurity: [
    {
      title: 'Penetration Test Report - {quarter} {year}',
      fileType: 'pdf',
      sizeRange: [300000, 1500000],
    },
    { title: 'Vulnerability Scan Results - {date}', fileType: 'csv', sizeRange: [50000, 500000] },
    {
      title: 'Security Incident Log - {month} {year}',
      fileType: 'xlsx',
      sizeRange: [30000, 200000],
    },
    { title: 'Access Control Matrix', fileType: 'xlsx', sizeRange: [40000, 180000] },
    {
      title: 'Firewall Configuration Backup - {date}',
      fileType: 'json',
...
const SAMPLE_DATA = {
  clients: [
    ...
    'Dominion Wealth Management',
    'Evergreen Asset Management',
    'Falcon Ridge Investments',
  ...
  matters: [
    'SEC Inquiry 2024-001',
    'Internal Investigation 2024-003',
    'Litigation Hold - Doe v. Fund',
    'Regulatory Exam 2024',
...
function getEvidenceCategory(controlId) {
  ...
  if (id.includes('accredited') || id.includes('investor-verification'))
    return 'accredited-investor';
  if (id.includes('disclosure') || id.includes('ppm') || id.includes('memorandum'))
    return 'disclosure';
  if (id.includes('communication') || id.includes('marketing') || id.includes('advertising'))
    return 'communication';
  if (id.includes('form-d') || id.includes('filing') || id.includes('edgar')) return 'form-d';
  if (id.includes('offering') || id.includes('subscription') || id.includes('investor-limit'))
    return 'offering';
  if (id.includes('integration') || id.includes('bad-actor') || id.includes('disqualification'))
    return 'integration';
  if (id.includes('solicitation') || id.includes('general-solicitation')) return 'solicitation';
  if (id.includes('retention') || id.includes('record') || id.includes('preservation'))
  ...
  if (
    ...
    return 'audit';
  if (id.includes('training') || id.includes('education') || id.includes('certification'))
    return 'training';
  if (
    id.includes('aml') ||
    id.includes('kyc') ||
...
function toSQL(evidence) {
  ...
  for (const e of evidence) {
    ...
    lines.push(
      `INSERT INTO evidence (id, control_id, artifact_hash, artifact_size, content_type, metadata, collected_at, collected_by, status)`
    );
    lines.push(
      `VALUES ('${e.id}', '${e.controlId}', '${e.artifactHash}', ${e.artifactSize}, '${e.contentType}', '${metadata}', '${e.collectedAt}', '${e.metadata.collectedBy}', '${e.status}');`
    );
    lines.push('');
  }
